...
//		foreach($this->Order->payoptions as );
		...
		$tag[$this->settings['label']] =  '<span class="billmate_partpayment"><span class="col1"><b>'.__('Invoice Information','shopp-billmate-partpayment').'</b><input type="text" id="pno" name="billmatepartpayment[pno]" class="required" value="'.$this->Order->partpaymentpno.'" /><label for="pno">'.__('Personal Number', 'shopp-billmate-partpayment').'</label></span><span class="col2" style="width:134px"><img src="'.content_url().'/shopp-addons/gateways/'.($this->module).'/images/bm_delbetalning_l.png"/></span><span style="width: 178px;height:99px;"><span style="padding: 0 !important;width: 264px;">'.__('Billmate Part Payment - Pay from','shopp-billmate-partpayment').' <span id="billmate_partpayment_price"></span>kr/'.__('month','shopp-billmate-partpayment').'</span><span><a id="terms-delbetalning">'.__('Terms of invoice','shopp-billmate-partpayment').'</a></span><input type="text" style="left:-999999px;position:absolute!important;" class="required" name="pclass" id="pclass" /></span><span style="min-width:425px;max-width: 480px;padding-right:none;"><input type="checkbox" checked="checked" name="billmatepartpayment[phone]" id="billmatepartphone" class="required" value="on" style="float: none;"><label for="billmatepartphone" style="float: right;width:95%" class="confirmlabel">'.__('My email address is correct and can be used for invoicing purposes.', 'shopp-billmate-invoice').'</label></span><input  type="image" src="'.content_url().'/shopp-addons/gateways/BillmatePartpayment/images/betala_delbetalning_knapp.gif" class="checkout-button" value="Submit Order" id="checkout-button" name="process"></span><style type="text/css">
